...
const exportedMethods = {
  ...
  checkPassword(strVal, varName) {
    ...
    if (!bval)
      throw `Error: ${varName} password must contain minimum eight characters, at least one letter, one number, and one special character @$!%*#?&`;
    return strVal;
  },
  /**
  ...
  checkAge(age, varName) {
    ...
    if (typeof age !== "number" || Number.isNaN(age) || !Number.isInteger(age))
      throw `Error: ${varName} must be an integer number!`;
  ...
  checkDifficulty(difficulty, varName) {
    ...
    if (difficulty < 1 || difficulty > 3)
      throw `Error: ${varName} must be 1=easy, 2=medium, 3=hard`;
    return difficulty;
  },
  ...
  checkStatus(status, varName) {
    ...
    if (status < 0 || status > 3)
      throw `Error: ${varName} must be to-do = 0, in-progress = 1, in-review = 2, completed = 3`;
    return status;
  },
